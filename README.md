...
function BlobmanBetaFeature.checkTarget()
    if not Settings.BlobmanBeta.selectedTarget then
        OrionLib:MakeNotification({Name = "エラー", Content = "ターゲットを選択してください", Time = 3})
        return false
    end
...
-- タブ一括作成（指定された順序）
-- ============================================
local GrabTab = Window:MakeTab({Name = "グラブ",Icon = "rbxassetid://7733955740",PremiumOnly = false})
local AntiTab = Window:MakeTab({Name = "アンチ",Icon = "rbxassetid://7734056411",PremiumOnly = false})
local BlobmanBetaTab = Window:MakeTab({Name = "ブロブマンベータ",Icon = "rbxassetid://7733916988",PremiumOnly = false})
local PvPTab = Window:MakeTab({Name = "PvP",Icon = "rbxassetid://7733771628",PremiumOnly = false})
local LoopTab = Window:MakeTab({Name = "ループ",Icon = "",PremiumOnly = false})
local VisualsTab = Window:MakeTab({Name = "ビジュアル", Icon = "rbxassetid://7733774602",PremiumOnly = false})
local AuraTab = Window:MakeTab({Name = "オーラ",Icon = "rbxassetid://7743868000",PremiumOnly = false})
local LineTab = Window:MakeTab({Name = "ライン",Icon = "rbxassetid://7734022107",PremiumOnly = false})
local ToyModTab = Window:MakeTab({Name = "トイモッド",Icon = "rbxassetid://7733956134",PremiumOnly = false})
local TrainTab = Window:MakeTab({Name = "トレインコントロール",Icon = "",PremiumOnly = false})
local MiscTab = Window:MakeTab({Name = "その他",Icon = "rbxassetid://7733970442",PremiumOnly = false})
local KeybindTab = Window:MakeTab({Name = "キーバインド",Icon = "rbxassetid://7743869317",PremiumOnly = false})

-- ============================================
-- グラブタブ
-- ============================================
GrabTab:AddSection({ Name = "グラブ強度設定" })
...
BlobmanBetaTab:AddButton({
    ...
    Callback = function()
        if Settings.BlobmanBeta.isExecuting then
            OrionLib:MakeNotification({Name = "実行中", Content = "既に実行中", Time = 3})
            return
        end
        ...
        local function executeKick()
            ...
            if not blob then
                ...
                if not blob then
                    OrionLib:MakeNotification({Name = "エラー", Content = "ブロブマンのスポーンに失敗しました", Time = 5})
                    return false
                end
            end
            ...
            if not BlobmanBetaFeature.isSittingOnBlobman() then
                ...
                if not BlobmanBetaFeature.isSittingOnBlobman() then
                    OrionLib:MakeNotification({Name = "エラー", Content = "ブロブマンに座っていません", Time = 4})
                    return false
                end
            end
            ...
            OrionLib:MakeNotification({Name = "完了", Content = "全員キックを実行しました", Time = 5})
            return true
        end
...
LoopTab:AddToggle({
    ...
    Callback = function(value)
        ...
        OrionLib:MakeNotification({
            Name    = "Whitelist",
            Content = "フレンドホワイトリスト: " .. (value and "有効" or "無効"),
            Image   = "rbxassetid://4483345998",
            Time    = 2,
        })
    end,
...
LoopTab:AddSection({ Name = "全員ブリング" })
...
LoopTab:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            OrionLib:MakeNotification({ Name = "全員ブリング", Content = "開始", Image = "rbxassetid://4483345998", Time = 2 })
        else
            LoopFeature.unFreezeCam()
            LoopFeature.stopFloating()
            OrionLib:MakeNotification({ Name = "全員ブリング", Content = "停止", Image = "rbxassetid://4483345998", Time = 2 })
        end
    end,
})

LoopTab:AddSection({ Name = "全員キル" })
...
LoopTab:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            ...
            OrionLib:MakeNotification({ Name = "全員キル", Content = "開始（近い順）", Image = "rbxassetid://4483345998", Time = 3 })
        else
            if Settings.Loop.killAllLoop then task.cancel(Settings.Loop.killAllLoop) Settings.Loop.killAllLoop = nil end
            LoopFeature.returnToOriginalPosition()
            Settings.Loop.OriginalPosition = nil
            Settings.Loop.ProcessedPlayers = {}
            LoopFeature.restoreCamera()
            OrionLib:MakeNotification({ Name = "全員キル", Content = "停止", Image = "rbxassetid://4483345998", Time = 2 })
        end
    end,
})

LoopTab:AddSection({ Name = "ターゲットキル" })

local TargetLabel = LoopTab:AddLabel("選択中: なし")
    ...
    Callback = function(value)
        ...
        if player then
            if Settings.Loop.TargetPlayers[player.UserId] then
                ...
                OrionLib:MakeNotification({ Name = "ターゲット削除", Content = player.Name .. " を削除しました", Image = "rbxassetid://4483345998", Time = 2 })
            else
                Settings.Loop.TargetPlayers[player.UserId] = true
                OrionLib:MakeNotification({ Name = "ターゲット追加", Content = player.Name .. " を追加しました", Image = "rbxassetid://4483345998", Time = 2 })
            end
            TargetLabel:Set("選択中: " .. LoopFeature.getSelectedTargetsDisplay())
        end
    end,
})
...
LoopTab:AddButton({
    ...
    Callback = function()
        ...
        OrionLib:MakeNotification({ Name = "更新完了", Content = "プレイヤーリストを更新しました", Image = "rbxassetid://4483345998", Time = 2 })
    end,
})
...
LoopTab:AddButton({
    ...
    Callback = function()
        ...
        TargetLabel:Set("選択中: なし")
        OrionLib:MakeNotification({ Name = "クリア完了", Content = "全ターゲットをクリアしました", Image = "rbxassetid://4483345998", Time = 2 })
    end,
})
    ...
    Callback = function(value)
        ...
        if value then
            if next(Settings.Loop.TargetPlayers) == nil then
                OrionLib:MakeNotification({ Name = "エラー", Content = "ターゲットを選択してください", Image = "rbxassetid://4483345998", Time = 3 })
            ...
            OrionLib:MakeNotification({ Name = "ターゲットキル", Content = "開始", Image = "rbxassetid://4483345998", Time = 3 })
        else
            if Settings.Loop.targetKillLoop then task.cancel(Settings.Loop.targetKillLoop) Settings.Loop.targetKillLoop = nil end
            LoopFeature.returnToOriginalPosition()
            Settings.Loop.OriginalPosition = nil
            OrionLib:MakeNotification({ Name = "ターゲットキル", Content = "停止", Image = "rbxassetid://4483345998", Time = 2 })
        end
    end,
})
...
VisualsTab:AddSection({ Name = "カメラ設定" })
...
VisualsTab:AddSection({ Name = "観戦設定" })
...
ToyModTab:AddToggle({Name = "家の物を使用", Default = false, Callback = function(v) Settings.ToyMod.UsePlotItems = v end})

ToyModTab:AddButton({Name = "翼を作成", Callback = function() ToyModFeature.CreateWings() end})
ToyModTab:AddButton({Name = "全翼を削除", Callback = function() ToyModFeature.DestroyWings() end})

ToyModTab:AddSection({Name = "配置"})
ToyModTab:AddSlider({Name = "羽根の間隔", Min = 0.05, Max = 5, Default = 0.1, Increment = 0.05, Callback = function(v) Settings.ToyMod.XSpacing = v end})
ToyModTab:AddSlider({Name = "Yオフセット", Min = -10, Max = 10, Default = 0, Increment = 0.5, Callback = function(v) Settings.ToyMod.YOffset = v end})
ToyModTab:AddSlider({Name = "Zオフセット", Min = -15, Max = 10, Default = 0, Increment = 0.5, Callback = function(v) Settings.ToyMod.ZOffset = v end})

ToyModTab:AddSection({Name = "回転"})
ToyModTab:AddSlider({Name = "水平角度", Min = -180, Max = 180, Default = -90, Increment = 5, ValueName = "°", Callback = function(v) Settings.ToyMod.AngleH = v end})
ToyModTab:AddSlider({Name = "垂直角度", Min = -90, Max = 90, Default = 0, Increment = 5, ValueName = "°", Callback = function(v) Settings.ToyMod.AngleV = v end})

ToyModTab:AddSection({Name = "羽ばたき"})
ToyModTab:AddToggle({Name = "有効", Default = false, Callback = function(v) Settings.ToyMod.FlapEnabled = v if not v then State.ToyModWings.FlapTime = 0 end end})
ToyModTab:AddDropdown({
    ...
    Callback = function(v) 
        for i = 1, 12 do
            local names = {
                "標準羽ばたき", "波状の翼", "上下浮遊", "ハチドリ", "スパイラル",
                "エレガント", "ツイスト", "パルス", "ジャンプ", "リボン",
                "フェニックス", "サイクロン"
            }
            if v == i .. ": " .. names[i] then
...
ToyModTab:AddSlider({Name = "速度（羽ばたき）", Min = 0.5, Max = 8, Default = 2, Increment = 0.1, Callback = function(v) Settings.ToyMod.FlapSpeed = v end})
ToyModTab:AddSlider({Name = "サイズ", Min = 10, Max = 300, Default = 30, Increment = 5, ValueName = "°", Callback = function(v) Settings.ToyMod.FlapRange = v end})

ToyModTab:AddSection({Name = "詳細設定"})
ToyModTab:AddToggle({Name = "速度リンク", Default = false, Callback = function(v) Settings.ToyMod.SpeedLinkFlap = v end})
...
MiscTab:AddSection({ Name = "パフォーマンス" })
...
MiscTab:AddSection({ Name = "ユーティリティ" })
...
KeybindTab:AddBind({
    ...
    Callback = function()
        ...
        local statusText = Settings.PvP.SilentAimEnabled and "オン" or "オフ"
        
        OrionLib:MakeNotification({
            Name = "サイレントエイム",
            Content = "サイレントエイム: " .. statusText,
            Image = "rbxassetid://4483345998",
            Time = 2
        })
    end    
